...
<template>
  <div class="max-w-4xl mx-auto space-y-6">
    ...
    <div>
      ...
      <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Manage your facility profile, billing, and security.</p>
    </div>
    ...
    <div v-if="activeSection === 'profile'" class="space-y-4">
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center gap-3">
          <div class="p-2 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-lg"><Building2 class="w-4 h-4" /></div>
          <div>
            <p class="font-bold text-slate-900 dark:text-slate-100 text-sm">Facility Details</p>
            <p class="text-xs text-slate-400 dark:text-slate-500">These details appear on printed tickets and receipts.</p>
          </div>
        </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              <label class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                ...
                <span class="ml-1 px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded text-[9px] font-black uppercase tracking-wider">Locked</span>
              </label>
              <input :value="store.profile.email" type="email" disabled
                class="w-full px-3 py-2.5 border border-slate-100 dark:border-slate-800 rounded-lg text-sm bg-slate-50 dark:bg-slate-800/50 text-slate-400 dark:text-slate-500 cursor-not-allowed" />
              <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Email is frozen after registration. Contact platform admin to change.</p>
            </div>
    ...
    <div v-else-if="activeSection === 'subscription'" class="space-y-4">
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center gap-3">
          <div class="p-2 bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-lg"><CreditCard class="w-4 h-4" /></div>
          <div>
            <p class="font-bold text-slate-900 dark:text-slate-100 text-sm">Subscription Plan</p>
            <p class="text-xs text-slate-400 dark:text-slate-500">Your current billing tier and account status.</p>
          </div>
        </div>
        ...
        <div class="p-6 space-y-5">
          ...
          <div class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
              <p class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Current Plan</p>
              <div class="flex items-center gap-3">
                <BadgeCheck class="w-7 h-7 text-blue-600 dark:text-blue-500" />
                <span class="text-2xl font-black text-slate-900 dark:text-slate-100">{{ store.profile.subscriptionPlan || 'BASIC' }}</span>
              </div>
            </div>
            <div class="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5">
              <p class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Account Status</p>
          ...
          <!-- Read-only notice -->
          <div class="flex items-start gap-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50 rounded-lg p-4">
            <AlertTriangle class="w-4 h-4 text-amber-500 flex-shrink-0 mt-0.5" />
            <div>
              <p class="text-sm font-bold text-amber-800 dark:text-amber-300">Plan changes require Platform Admin</p>
              <p class="text-xs text-amber-700 dark:text-amber-400 mt-0.5">
                Your <code class="bg-amber-100 dark:bg-amber-900/50 px-1 rounded">subscriptionPlan</code> and
                <code class="bg-amber-100 dark:bg-amber-900/50 px-1 rounded">subscriptionStatus</code> fields are
          ...
          <!-- Capacity -->
          <div class="border border-dashed border-slate-300 dark:border-slate-600 rounded-lg p-4 flex justify-between items-center">
            <div>
              <p class="text-sm font-bold text-slate-700 dark:text-slate-300">Total Parking Capacity</p>
              <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Set during tenant provisioning. Contact admin to adjust.</p>
            </div>
            <span class="text-2xl font-black text-slate-900 dark:text-slate-100">—</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="activeSection === 'security'" class="space-y-4">
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center gap-3">
          <div class="p-2 bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-lg"><ShieldCheck class="w-4 h-4" /></div>
          <div>
            <p class="font-bold text-slate-900 dark:text-slate-100 text-sm">Security Overview</p>
            <p class="text-xs text-slate-400 dark:text-slate-500">Your session and access information.</p>
          </div>
        </div>
        ...
        <div class="p-6 space-y-4">
          ...
          <div>
            <p class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Server-Protected Fields</p>
            <div class="space-y-2">
              <div v-for="field in [
                ...
                class="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg">
                <Lock class="w-4 h-4 text-slate-400 dark:text-slate-500 flex-shrink-0 mt-0.5" />
                <div>
                  <code class="text-xs font-black text-slate-700 dark:text-slate-300">{{ field.name }}</code>
                  <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ field.reason }}</p>
                </div>
              </div>
            </div>
