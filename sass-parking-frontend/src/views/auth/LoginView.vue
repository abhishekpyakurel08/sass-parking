...
<template>
  <div class="min-h-screen w-full flex bg-slate-50 dark:bg-slate-900 font-sans">
    
    <!-- LEFT PANEL: Dynamic Auth -->
    <div class="w-full lg:w-[45%] xl:w-5/12 flex flex-col justify-center px-8 sm:px-16 lg:px-20 relative overflow-y-auto bg-white dark:bg-slate-800 shadow-2xl z-10 py-16">
      ...
      <div class="absolute top-8 left-8 sm:left-16 lg:left-20 flex items-center gap-3">
        ...
        <span class="font-bold text-xl tracking-tight text-slate-900 dark:text-slate-100">ParkSaaS<span class="text-emerald-600 dark:text-emerald-500">Pro</span></span>
      </div>

      <div class="w-full max-w-sm mx-auto mt-20">
        
        <!-- Toggle Tabs with sliding indicator -->
        <div class="flex p-1.5 bg-slate-100/80 dark:bg-slate-700/80 backdrop-blur-sm rounded-xl mb-10 relative">
          <div class="absolute inset-y-1.5 w-[calc(50%-6px)] bg-white dark:bg-slate-600 rounded-lg shadow-[0_2px_8px_-2px_rgba(0,0,0,0.12)] transition-all duration-500 cubic-bezier-out"
          ...
          <button @click="toggleView('login')" 
                  ...
                  :class="view === 'login' ? 'text-slate-900 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'">
            Login
          </button>
        ...
        <div class="relative min-h-[400px]">
          <Transition name="slide-fade" mode="out-in">
              ...
              <div class="space-y-5 pt-4">
                ...
                <div class="space-y-1.5">
                  ...
                  <div class="relative group">
                    ...
                    <button type="button" @click="showLoginPassword = !showLoginPassword" class="absolute right-3.5 top-3 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 focus:outline-none transition-colors">
                ...
                <div class="flex items-center gap-2.5 pt-1">
                  <input type="checkbox" id="rememberMe" v-model="loginData.rememberMe" 
                         class="rounded-md border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-emerald-600 focus:ring-emerald-500 w-4 h-4 cursor-pointer" />
                  <label for="rememberMe" class="text-sm font-medium text-slate-600 dark:text-slate-400 cursor-pointer select-none">Keep session active (30 days)</label>
                </div>
              </div>
            ...
            <form v-else-if="view === 'register'" @submit.prevent="onSubmit" class="space-y-6 w-full absolute inset-0">
              <div class="space-y-2">
                <h2 class="text-3xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">Deploy Node.</h2>
                <p class="text-slate-500 dark:text-slate-400">Initialize a new parking management instance.</p>
              </div>

              <div class="space-y-5 pt-4">
                <div class="space-y-1.5">
                  <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Business Name</label>
                  <div class="relative group">
                    <Building2 class="absolute left-3.5 top-3 h-5 w-5 text-slate-400 dark:text-slate-500 group-focus-within:text-emerald-500 transition-colors" />
                ...
                <div class="space-y-1.5">
                  <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">URL Slug <span class="text-slate-400 dark:text-slate-500 font-normal">(e.g. acme-parking)</span></label>
                  <input v-model="registerData.slug" type="text" placeholder="acme-parking" required
                         class="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 font-medium lowercase" />
                </div>

                <div class="space-y-1.5">
                  <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Owner Name</label>
                  <input v-model="registerData.ownerName" type="text" placeholder="John Doe" required
                         class="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 font-medium" />
                </div>

                <div class="space-y-1.5">
                  <label class="text-sm font-semibold text-slate-700 dark:text-slate-300">Admin Contact Email</label>
                  <div class="relative group">
                    <Mail class="absolute left-3.5 top-3 h-5 w-5 text-slate-400 dark:text-slate-500 group-focus-within:text-emerald-500 transition-colors" />
                ...
                <div class="space-y-1.5">
                  ...
                  <div class="relative group">
                    ...
                    <input v-model="registerData.password" :type="showRegisterPassword ? 'text' : 'password'" placeholder="••••••••" required minlength="8"
                           class="w-full pl-11 pr-11 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none text-slate-900 dark:text-slate-100 font-medium tracking-widest" />
                    <button type="button" @click="showRegisterPassword = !showRegisterPassword" class="absolute right-3.5 top-3 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 focus:outline-none transition-colors">
      ...
      <!-- Footer details -->
      <div class="absolute bottom-8 left-8 sm:left-16 lg:left-20 text-xs text-slate-400 dark:text-slate-500 font-medium tracking-wide">
        &copy; 2026 ParkSaaS Systems Inc. &middot; <a href="#" class="hover:text-slate-600 dark:hover:text-slate-300 transition-colors">Privacy</a>
      </div>
    </div>
