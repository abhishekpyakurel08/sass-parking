...
const loginData = reactive({ email: "", password: "", remember: false })
const registerData = reactive({ companyName: "", email: "", password: "" })
...
<template>
  <div class="min-h-screen w-full flex bg-slate-50 font-sans">
    ...
    <div class="w-full lg:w-[45%] xl:w-5/12 flex flex-col justify-center px-8 sm:px-16 lg:px-20 relative overflow-hidden bg-white shadow-2xl z-10">
      ...
      <div class="w-full max-w-sm mx-auto mt-20">
        ...
        <div class="flex p-1.5 bg-slate-100/80 backdrop-blur-sm rounded-xl mb-10 relative">
          ...
          <div class="absolute inset-y-1.5 w-[calc(50%-6px)] bg-white rounded-lg shadow-[0_2px_8px_-2px_rgba(0,0,0,0.12)] transition-all duration-500 cubic-bezier-out"
               :class="view === 'login' ? 'left-1.5' : 'left-[calc(50%+4px)]'"></div>
        ...
        <div class="relative min-h-[400px]">
          <Transition name="slide-fade" mode="out-in">
            ...
            <form v-if="view === 'login'" @submit.prevent="onSubmit" class="space-y-6 w-full absolute inset-0">
              ...
              <div class="space-y-5 pt-4">
                ...
                <div class="flex items-center gap-2.5 pt-1">
                  ...
                  <label for="remember" class="text-sm font-medium text-slate-600 cursor-pointer select-none">Keep session active</label>
                </div>
              </div>
            ...
            <form v-else-if="view === 'register'" @submit.prevent="onSubmit" class="space-y-6 w-full absolute inset-0">
              ...
              <div class="space-y-5 pt-4">
                <div class="space-y-1.5">
                  <label class="text-sm font-semibold text-slate-700">Facility / Company Name</label>
                  <div class="relative group">
                    <Building2 class="absolute left-3.5 top-3 h-5 w-5 text-slate-400 group-focus-within:text-emerald-500 transition-colors" />
                    <input v-model="registerData.companyName" type="text" placeholder="Acme Parking Ltd." required
                           class="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none text-slate-900 placeholder:text-slate-400 font-medium" />
                  </div>
                </div>
                ...
                <div class="space-y-1.5">
                  ...
                  <div class="relative group">
                    <Mail class="absolute left-3.5 top-3 h-5 w-5 text-slate-400 group-focus-within:text-emerald-500 transition-colors" />
                    <input v-model="registerData.email" type="email" placeholder="admin@acmeparking.com" required
                           class="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none text-slate-900 placeholder:text-slate-400 font-medium" />
                  </div>
                </div>
