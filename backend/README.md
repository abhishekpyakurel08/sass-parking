...
1. **`SUPER_ADMIN`**: Global platform administrators. Can manage subscriptions, suspend tenants, and view system-wide analytics. They are *not* bound to a single `tenant_id`.
...
Critical financial workflows (e.g., `processPayment`, `lostTicket`) utilize **MongoDB Transactions** (`session.startTransaction()`). This ensures that if the system crashes while updating a ticket's status to `PAID`, but *before* it updates the customer's loyalty discount, the entire database action rolls back cleanly without leaving corrupted half-states.
