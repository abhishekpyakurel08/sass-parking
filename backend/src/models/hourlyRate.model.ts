...
export interface IHourlyRate {
  ...
  grace_period_minutes: number; // New field for grace period
  createdAt: Date;
  updatedAt: Date;
}
...
const HourlyRateSchema = new Schema<IHourlyRate>(
  {
    ...
    grace_period_minutes: { type: Number, required: true, min: 0, default: 0 }, // Set default to 0 for no grace period
  },
  { timestamps: true }
);
