 ...
 * It waits for the response to finish, and if the mutation was successful (2xx), it logs it.
 */
export const auditAction = (resource: string) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    res.on('finish', () => {
      // Only log state-changing methods or explicitly targeted requests
      if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(req.method) && res.statusCode >= 200 && res.statusCode < 300) {
        if (req.user && req.user.tenantId && req.user.userId) {
          // Exclude sensitive data from body
